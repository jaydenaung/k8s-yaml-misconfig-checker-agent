...
- **No security guarantee** — A clean report does not mean your cluster is secure. Static analysis and AI-assisted checks can miss issues. Always combine this tool with manual review, penetration testing, and defence-in-depth practices.
- **AI findings require human review** — Findings marked `[AI]` are generated by a large language model. They may contain false positives or false negatives. Never act on an AI finding without independent verification.
- **No warranty** — This software is provided "as is", without warranty of any kind, express or implied. The author accepts no liability for damages of any kind arising from its use.
- **Use at your own risk** — You are solely responsible for decisions made based on this tool's output. The author is not liable for any production incidents, security breaches, compliance failures, or other consequences resulting from use of this tool.
- **Third-party tools** — KubeSentinel optionally invokes `kubectl`, `helm`, and `trivy`. You are responsible for their installation, configuration, and the permissions granted to them.
- **Untrusted input** — Do not run KubeSentinel against YAML files from untrusted sources without reviewing them first. Malicious YAML could contain text designed to manipulate the AI analysis (prompt injection).

> **TL;DR:** This is a helper tool, not a compliance auditor. It surfaces issues for your engineers to review — it does not replace human judgment or formal security assessments.
